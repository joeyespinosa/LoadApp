...
class MainActivity : AppCompatActivity() {
    ...
    private var downloadItemId: Long = 0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        loadingButton = findViewById(R.id.button_loading)
                    ...
                    if (cursor.moveToFirst()) {
                        if (cursor.count > 0) {
                            ...
                            if (status == DownloadManager.STATUS_SUCCESSFUL) {
                                loadingButton.setLoadingButtonState(ButtonState.Completed)
                                notificationManager.createNotificationItem(
    ...
    private fun downloadItem() {
        ...
        if (selectedGitHubRepository != null) {
            ...
            createChannel(getString(R.string.github_notification_channel_id), getString(R.string.github_notification_channel_name))
            ...
            val request = DownloadManager.Request(Uri.parse(selectedGitHubRepository))
                    ...
                    .setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, "/repos/repositoryitem.zip")

            val downloadManager = getSystemService(DOWNLOAD_SERVICE) as DownloadManager
            downloadItemId = downloadManager.enqueue(request)
        } else {
            loadingButton.setLoadingButtonState(ButtonState.Completed)
            showToast(getString(R.string.no_selected_item))
        }
    }
        ...
        if (view is RadioButton) {
            ...
            when (view.getId()) {
                R.id.radiobutton_glide ->
                    if (isChecked) {
                        selectedGitHubRepository = getString(R.string.glide_url)
                        selectedGitHubFileName = getString(R.string.glide_text)
                    }

                R.id.radiobutton_load_app ->
                    if (isChecked) {
                        selectedGitHubRepository = getString(R.string.load_app_url)
                        selectedGitHubFileName = getString(R.string.load_app_text)
                    }

                R.id.radiobutton_retrofit -> {
                    if (isChecked) {
                        selectedGitHubRepository = getString(R.string.retrofit_url)
...
fun NotificationManager.createNotificationItem(
        messageBody: String,
    ...
    val notificationBuilder = NotificationCompat.Builder(applicationContext, applicationContext.getString(R.string.github_notification_channel_id))
            .setSmallIcon(R.drawable.ic_assistant_black_24dp)
            .setContentTitle("Download Item Done")
